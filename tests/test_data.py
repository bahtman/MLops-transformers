...
# '/Users/frederikkjaer/Documents/DTU/8Semester/MLOps/dtu_mlops/Project/MLops-transformers/data/processed/training.pt')
# trainset = torch.utils.data.TensorDataset(*trainset)
# test_data, test_label = torch.load(
# '/Users/frederikkjaer/Documents/DTU/8Semester/MLOps/dtu_mlops/Project/MLops-transformers/data/processed/test.pt')
...
@pytest.mark.parametrize("test_input,expected",
                       [("train_data", torch.Size([4179, 2, 768])),
                        ("test_data", torch.Size([1393, 2, 768]))])
def test_data_shape(test_input, expected):
    assert eval(test_input)[0].shape == expected


@pytest.mark.parametrize("test_input,expected",
                       [("train_label", np.arange(2)),
                        ("test_label", np.arange(2))])
